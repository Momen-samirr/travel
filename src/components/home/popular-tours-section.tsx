...
import { MapPin, Calendar, Star, ArrowRight, Package } from "lucide-react";
...
export function PopularToursSection({ packages, tours }: PopularToursSectionProps) {
  const items = packages || tours || [];
  if (items.length === 0) return null;

  const isPackages = !!packages;
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              <h2 className="text-4xl font-bold mb-2">
                {isPackages ? "Popular Charter Packages" : "Popular Tours"}
              </h2>
              <p className="text-lg text-muted-foreground">
                {isPackages
                  ? "Complete travel bundles with flights, hotels, and more"
                  : "Handpicked destinations for your next adventure"}
              </p>
            </div>
            <Button asChild variant="outline" className="hidden md:flex">
              <Link href={isPackages ? "/charter-packages" : "/tours"}>
                View All {isPackages ? "Packages" : "Tours"}
        ...
        <StaggerList className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.slice(0, 6).map((item) => {
            if (isPackages) {
              const pkg = item as CharterPackage;
              ...
              const price = tour.discountPrice
                    ...
                    <Card className="card-hover overflow-hidden group h-full">
                      <div className="relative h-64 w-full overflow-hidden">
                        ...
                        {tour.discountPrice && (
                          <Badge className="absolute top-4 right-4 bg-primary text-primary-foreground">
                            {Math.round(
                              ((Number(tour.price) - Number(tour.discountPrice)) /
                                Number(tour.price)) *
                                100
                            )}
                            % Off
