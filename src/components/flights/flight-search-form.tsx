...
export function FlightSearchForm({
  ...
  const [adults, setAdults] = useState(
    initialValues?.adults?.toString() || "1",
  );
  const [children, setChildren] = useState(
    initialValues?.children?.toString() || "0",
  );
  const [infants, setInfants] = useState(
    initialValues?.infants?.toString() || "0",
  );
  ...
  const validate = (): boolean => {
    ...
    if (!destination) {
      ...
      newErrors.destination =
        "Destination must be a valid 3-character airport code";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log(
      "[FlightSearchForm] Submitting search with params:",
      searchParams,
    );
